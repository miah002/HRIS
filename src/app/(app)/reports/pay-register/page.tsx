          ...
          <Table>
            <TableHeader>
              <TableRow>
                <Th>Employee</Th>
                <Th>Dept</Th>
                <Th className="text-center w-14">Days</Th>
                <Th numeric>Basic</Th>
                <Th numeric>OT</Th>
                <Th numeric>ND</Th>
            ...
            <TableBody>
              {rows.map(({ employee: e, payroll: p, totalDed }) => (
                <TableRow key={e.id}>
                  <Td>
                  ...
                  <Td className="text-center text-xs">{p ? p.daysWorked.toFixed(1) : "—"}</Td>
                  <Td numeric className="text-xs">{p ? php(p.basicPay) : "—"}</Td>
                  <Td numeric className="text-xs">{p ? php(p.overtimePay) : "—"}</Td>
                  <Td numeric className="text-xs">{p ? php(p.nightDiffPay) : "—"}</Td>
                  <Td numeric className="text-xs">{p ? php(p.holidayPay) : "—"}</Td>
                  <Td numeric className="font-semibold text-xs">{p ? php(p.grossPay) : "—"}</Td>
                  <Td numeric className="text-xs text-[var(--text-secondary)]">
                    {p ? (
                      <span title={`SSS ${php(p.sssEE)} · PhilHealth ${php(p.philHealthEE)} · HDMF ${php(p.pagIbigEE)} · WHT ${php(p.withholdingTax)} · Loans ${php(p.loanDeductions)}`}>
                        {php(totalDed)}
                      </span>
                    ) : "—"}
                  </Td>
                  <Td numeric className="font-bold text-xs text-[var(--brand)]">{p ? php(p.netPay) : "—"}</Td>
                </TableRow>
              ))}
            </TableBody>
            <TableFooter>
              <TableRow>
                <Td colSpan={3} className="text-xs font-semibold">
                  Totals — {rows.filter(r => r.payroll).length} of {rows.length} employees
                </Td>
                ...
                <Td numeric className="font-bold text-xs">{php(totals.gross)}</Td>
                <Td numeric className="font-semibold text-xs">{php(totals.ded)}</Td>
                <Td numeric className="font-bold text-xs text-[var(--brand)]">{php(totals.net)}</Td>
              </TableRow>
            </TableFooter>
          </Table>
        </CardContent>
